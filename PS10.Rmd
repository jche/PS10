...
Finally, we create the submission files.

```{r}
LASSO_preds <- predict(LASSO_fit, newx=predictor_matrix_test, s=lambda_star_1SE)
LASSO_submission <- sample_submission %>%
  mutate(price_doc = LASSO_preds)

lm <- lm(model_formula, data=train)
lm_preds <- predict(lm, test)
lm_submission <- sample_submission %>%
  mutate(price_doc = lm_preds)
